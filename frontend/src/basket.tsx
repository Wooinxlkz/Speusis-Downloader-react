...
import { ShoppingBasket, X, Plus, Check, CircleAlert, RotateCw, Trash2 } from "lucide-react";
...
function BasketApp() {
  ...
  // Real theme/accent sync - this window is its own separate document, so
  // nothing here was ever theme-aware before; it always rendered in the
  // default light palette regardless of the app's actual setting. Synced
  // at mount and re-synced whenever the window regains focus (it's
  // hidden/shown rather than destroyed between opens, so a mount-only
  // fetch would go stale if the theme changed while it was hidden).
  useEffect(() => {
    const sync = () => ipc.settingsGet().then(applyTheme).catch(() => {});
    sync();
    window.addEventListener("focus", sync);
    return () => window.removeEventListener("focus", sync);
  }, []);

  // Re-render relative timestamps every 15s without needing new data.
  useEffect(() => {
  ...
  return (
    <div className="flex h-screen w-screen flex-col bg-bg text-ink motion-modal-in">
      <div data-tauri-drag-region className="flex h-9 flex-shrink-0 items-center gap-2 border-b border-line-soft bg-panel px-3">
        ...
        {status ? (
          <span className="ml-2 text-[10.5px] text-faint">{status}</span>
        ) : recent.length > 0 ? (
          <span className="ml-2 rounded-full bg-sunken px-1.5 text-[10px] text-faint">{recent.length}</span>
        ) : null}
        <button
      ...
      <div className="flex flex-1 flex-col gap-2.5 overflow-hidden p-2.5">
        {!formOpen ? (
          <div
            ...
            className={`flex flex-shrink-0 cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-[1.5px] border-dashed py-6 text-center transition-colors ${
