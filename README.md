...
**Current: 0.1.6.** Full version-by-version changelog lives in
[`RELEASES.md`](./RELEASES.md) — every release, what changed, and every
real bug found and fixed along the way. Bumped across
`frontend/package.json`, `src-tauri/Cargo.toml`, and `tauri.conf.json`
on every release. `speusis-core` stays at 0.4.6, still unmodified except
the one `update_checker.rs` line from 0.1.2 (see below).
...
- **Language coverage is real but partial.** The switcher loads the
...
- **Batch is paste-based, not tab-scanning.** The old app's real "Batch"
  panel scanned the browser's active tab via the extension bridge — not
  practically reproducible without a live extension connection. The
  paste-based version here is a genuine substitute, not a 1:1 match.
- **Move/Rename and Tracer** have no backend command in the current
  engine — both are shown in menus but honestly disabled rather than
  faked.
